...
try:
    from .schema import (
        TOPAOutput, Summary, TestCounts, FileCounts, FileSummary, 
        TestResult, FileIssues, TestStatus, TestType,
        ParsedTestData, ParsedFileResult, ParsedTestResult
    )
...
class TOPAEncoder:
    ...
    def __init__(self, focus_mode: str = FocusMode.FAILURES, token_budget: Optional[TokenBudget] = None):
    ...
    def _build_files_with_issues(self, parsed_data: ParsedTestData) -> List[FileIssues]:
        ...
        for file_result in parsed_data.file_results:
            if file_result.has_issues():
                ...
                files_with_issues.append(FileIssues(
    ...
    def _build_critical_failures(self, parsed_data: ParsedTestData) -> List[FileSummary]:
            ...
            if error_tests:
                ...
                for test in error_tests:
                    test_result = TestResult(
                        ...
                        name=test.name,
                        type=TestType.ERROR,
                        error=self.budget.smart_truncate(test.error_message or "unknown error", 50)
                    )
    ...
    def _build_first_failure_details(self, parsed_data: ParsedTestData) -> List[FileSummary]:
        ...
        for file_result in parsed_data.file_results:
            ...
            if failed_tests:
                ...
                truncated_count = len(failed_tests) - 1 if len(failed_tests) > 1 else None
    ...
    def _build_all_failure_details(self, parsed_data: ParsedTestData) -> List[FileSummary]:
    ...
    def _build_test_result(self, test: ParsedTestResult) -> TestResult:
        ...
        if test.is_error:
            ...
            if (self.budget.remaining > 100 and 
                test.expected and test.actual and
                isinstance(test.expected, str) and isinstance(test.actual, str)):
