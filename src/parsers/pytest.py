...
class PytestParser(BaseParser):
    ...
    def parse(self, content: str) -> ParsedTestData:
        ...
        if total_tests > 0 and not file_results:
            ...
            for i in range(failed_count):
                generic_tests.append(ParsedTestResult(
            ...
            file_results.append(ParsedFileResult(
                file_path="pytest_output",
                test_results=generic_tests
            ))
    ...
    def _extract_assertion_from_context(self, lines: List[str], start_index: int) -> Optional[tuple]:
        ...
        for i in range(start_index, min(start_index + 10, len(lines))):
            ...
            if any(pattern.match(line) for pattern in [
                self.failure_pattern, self.error_pattern, self.passed_pattern
            ]):
                break
