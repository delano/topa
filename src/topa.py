...
try:
    from .core.schema import TOPAOutput, TestResult, FileSummary  
...
def detect_format(content: str) -> InputFormat:
    ...
    pytest_patterns = [
        'failed', 'passed', '::test_', 'collected',
        'assertions', 'traceback', 'assert '
    ]
  ...
  topa --format rspec --limit 1000 rspec.json
    ...
    try:
        ...
        try:
            ...
            print(f"Error parsing {input_format.value} format: {e}", file=sys.stderr)
            sys.exit(1)
