...
class RSpecParser(BaseParser):
        ...
        if not passed:
            ...
            if exception:
                ...
                if self._is_rspec_error(exception_class):
                    ...
                    test_result.error_message = f"{exception_class}: {exception_message}".strip(': ')
                else:
                    ...
                    if expected and actual:
                        ...
                        test_result.actual = f"{exception_class}: {exception_message}".strip(': ')
    ...
    def _parse_as_text(self, content: str, error_context: str) -> ParsedTestData:
        ...
        for line in lines:
            ...
            if any(pattern in line.lower() for pattern in ['example', 'spec', 'failure', 'error']):
                test_name = line
        ...
        file_results = [ParsedFileResult(
            file_path="rspec_parse_error.json",
