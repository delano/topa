...
try:
    ...
    from ..core.schema import ParsedTestData, ParsedFileResult, ParsedTestResult
except ImportError:
...
class TAPParser(BaseParser):
    ...
    def __init__(self):
        ...
        self.test_pattern = re.compile(r'^(ok|not ok)(?:\s+(\d+))?(?:\s*-?\s*(.*))?$', re.IGNORECASE)
        self.directive_pattern = re.compile(r'#\s*(SKIP|TODO|FIXME)(?:\s+(.*))?$', re.IGNORECASE)
    ...
    def parse(self, content: str) -> ParsedTestData:
        ...
        for line_num, line in enumerate(lines):
            ...
            if test_match:
                ...
                test_number = int(test_match.group(2)) if test_match.group(2) else len(test_results) + 1
                ...
                if directive == 'TODO':
                    ...
                    if passed:
                        ...
                        pending_diagnostics.append("Unexpected pass - TODO item succeeded")
                    else:
                ...
                if not passed and pending_diagnostics:
                    ...
                    if self._is_error_message(diagnostic_text):
                        ...
                        expected, actual = self._extract_assertion_values(diagnostic_text)
            ...
            if diagnostic_match:
                ...
                if not diagnostic or diagnostic.upper().startswith(('SKIP', 'TODO', 'FIXME')):
                    continue
        ...
        if planned_tests > 0 and total_tests != planned_tests:
            ...
            test_results.append(ParsedTestResult(
                name=f"test plan mismatch",
                passed=False,
